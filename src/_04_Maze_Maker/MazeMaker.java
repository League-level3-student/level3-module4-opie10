...
public class MazeMaker {
	...
	public static Maze generateMaze(int r, int c) {
		...
		Cell start = maze.getCell(ranrow, rancol);
		Cell end = maze.getCell(ranerow, ranecol);
		if (ranrow == 0) {
		...
		if (ranerow == 0) {
			end.setNorthWall(false);
		} else if (ranerow == 4) {
			end.setSouthWall(false);
		} else if (ranecol == 0) {
			end.setWestWall(false);
		} else if (ranecol == 4) {
		...
		Cell randcell = maze.getCell(randcr, randcc);
